...
class Nets:
    ...
    def add_route(
        self, layer=None, x1=None, y1=None, z1=None, x2=None, y2=None, z2=None, via=None
    ):
    ...
    def report_route_graph(self):
        ...
        # return nx.node_link_graph(self.route)

    def report_route_graph_dict(self):
        base = {}

    def compress_routes(
        self, debug=True, design="", report_route=True, subsegment=True
    ):
        ...
        def subsegment_routes(route, d_routes):
            ...
            in_routes = []
            in_routes.append(
                {"route": route, "head": False, "tail": False, "break": []}
            )
            for dr in d_routes:
                in_routes.append(
            ...
            for ind_ends, dr_ends in enumerate(in_routes):
                for ind_srch, dr_srch in enumerate(in_routes):
                    if (
                        ind_ends == ind_srch
                    ):  # this means we are checking the same route, skip
                        continue
                    ...
                    if (
                        in_routes[ind_ends]["head"] and in_routes[ind_ends]["tail"]
            ...
            net_G = nx.Graph()
            if debug:
                print(f"Subsegment net {self.net}")
            ...
            for ind, r_t in enumerate(in_routes):
                ...
                for br_pt in r_t["break"]:
                    ...
                    if new_node in net_G.nodes:
                        net_G.nodes[new_node]["route"] = r_t["route"][
                            last_br_ind : br_pt["pt_ind"] + 1
                        ]
                    else:
                        net_G.add_node(
                            f"{ind}_{br_count}",
                    ...
                    for ch_end in list(br_pt["r_ind"]):
                        ...
                        if ch_end[1] == "head":
                            ...
                            ch_end[1] == "tail"
                        ):  # we assume last seg is # of break pts
                            net_G.add_edge(
                    ...
                    if ind == len(r_t["break"]) - 1:
                        ...
                        net_G.nodes[f"{ind}_{br_count+1}"]["route"] = r_t["route"][
                            last_br_ind : len(r_t["route"])
                        ]
                        if debug:
                            print(
        ...
        while True:
            ...
            if debug:
                print("sr:" + str(len(self.route)) + ":" + str(self.route))
    ...
    #
    #         def can_insert(self):
    #             return self.head_link is not None and self.tail_link is not None
    #
...
class NetBuilder:
    ...
    def __init__(
        self, px=None, layer=None, lpv=None, def_scale=None, bottom_layers=None
    ):
    ...
    def import_tlef(self, tlef_f):
        ...
        with open(tlef_f, "r+") as f:
            ...
            for l in mo_l:
                self.layers_list[l.group("type").decode("utf-8")].append(
                    l.group("layer_name").decode("utf-8")
                )
...
class Component:
    ...
    def __init__(self, name=None, comp=None, x1=None, y1=None, dir=None):
...
class Pin:
    ...
    def set_connect_dir(self, cdir):
        if cdir.upper() == "TOP" or connect_dir == "z+":
            self.connect_dir = "z+"
        elif cdir.upper() == "BOTTOM" or connect_dir == "z-":
            self.connect_dir = "z-"
        elif cdir.upper() == "LEFT" or connect_dir == "x+":
            self.connect_dir = "x+"
        elif cdir.upper() == "RIGHT" or connect_dir == "x-":
            self.connect_dir = "x-"
        elif cdir.upper() == "FRONT" or connect_dir == "y+":
            self.connect_dir = "y+"
        elif cdir.upper() == "BACK" or connect_dir == "y-":
            self.connect_dir = "y-"
